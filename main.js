...
async function filesInit() {
	...
	try {
		...
		for (let file of CommandsFiles) {
			try {
				const relativePath = path.relative(path.join(__dirname, directory), file);
				const module = await import(`${directory}/${relativePath}`);
				global.plugins[`${directory}/${relativePath}`] = module.default || module;
			} catch (e) {
				conn.logger.error(e);
