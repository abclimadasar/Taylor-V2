...
switch (template) {
        ...
        return conn.sendFile(m.chat, nsf, '', args[0], m)
            break
            case 'anime':
case 'animedl1':
case 'animedl2':
...
let linkq = 'https://mfarels.my.id/api/' + args[0] + '?q=' + one
        ...
        return conn.sendFile(m.chat, nofeq, '', args[0], m)
        }
        if (!scanq == "application/json") {
        return conn.sendFile(m.chat, linkq, '', args[0], m)
        }
            break
            case 'bahasa-g':
case 'bajinganlo':
...
let linkt = 'https://mfarels.my.id/api/' + args[0] + '?text=' + one
        ...
        return conn.reply(m.chat, nofet, m)
        }
        if (!scan == "application/json") {
        return conn.sendFile(m.chat, linkt, '', '', m)
        }
            break
}
}
